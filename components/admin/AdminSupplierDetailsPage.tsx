...
import { BoxOpenIcon, ChartUpIcon, MoneyBillTransferIcon, WarehouseIcon, CheckCircleIcon, WalletIcon, PlusIcon, XMarkIcon } from '../common/icons';
                        ...
                        {products.filter(p => p.supplierId === supplier.id).map(product => (
                            <div key={product.id} className="flex items-center gap-3 p-2 border-b dark:border-gray-700 last:border-0">
                                ...
                                <div className="text-left">
                                    <span className="text-xs font-bold bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">مخزون: {product.stock}</span>
                                </div>
                            </div>
                        ))}
